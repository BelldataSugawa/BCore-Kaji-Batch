...
namespace BAT240
{
    internal class BAT240
    {
        static int Main(string[] args)
        {
            ...
            CsvTable.Columns.Add("NOCODE", typeof(string));     //コード番号
            CsvTable.Columns.Add("NOCDE1", typeof(string));     //枝番
            CsvTable.Columns.Add("NOYSRV", typeof(string));     //実行予算リビジョン
            CsvTable.Columns.Add("KBSTKN", typeof(string));     //完成状況
            CsvTable.Columns.Add("WorKFlag", typeof(string));   //完成フラグ
            //連携ステータス
            string RnkStatus = "1";
            //日付
            string RnkDate = DateTime.Now.ToString("yyyyMMddHHmmss");
            //CSVファイル
            string CsvFile = "";
            //ワークパス
            string WorkPath = "";
            ...
            try
            {
                ...
                using (var conn = new OdbcConnection(connStr))
                {
                    ...
                    {
                        //取得したデータを変数にセット
                        var CDKUBN = KBNread["CDKUBN"].ToString();
                        var VLKUBN = KBNread["VLKUBN"].ToString();
                        var TXMJT1 = KBNread["TXMJT1"].ToString();
                        var TXMJT2 = KBNread["TXMJT2"].ToString();
                        ...
                        switch (CDKUBN)
                        {
                            ...
                            case "TXCDRN":
                                ...
                                else
                                {
                                    ...
                                    //TXMJT1、TXMJT2どちらも空でない場合のみセットする
                                    if (ConfigurationManager.AppSettings["EnvMode"] == "PRODUCTION")
                                    {
                                        if (TXMJT1 != "")
                                            WorkPath = WorkPath + TXMJT1;
                                    }
                                    else
                                    if (TXMJT2 != "")
                                    {
                                        WorkPath = WorkPath + TXMJT2;
                                    }
                                }
                                break;
                        }
            ...
            //未送信データが０件、DB接続エラー、エラーの場合、戻り値9を返す
            if (FHZYNreturn == null)
            {
                return 9;                          //戻り値9を返す
            }
            ...
            //2-4-1
            //未送信データの有無チェック
            if (Flist.Count == 0)                 //未送信データ0の場合戻り値1を返す
            {
                return 1;
            }
            else if (Flist.Count > 0)             //未送信データ有の場合、完成フラグを追加する
            {
                int count = 0;
                ...
                foreach (var item in Flist)
                {
                    ...
                    Console.WriteLine($"2-4-1WorkFlag追加====コード番号:{item.NOCODE}, 枝番:{item.NOCDE1},リビジョン:{item.NOYSRV}, 完了状況:{item.KBSTKN}, 完了フラグ:{item.WorkFlag}");

                    //2-4-2 CSVテーブルにコード番号と完成フラグを追加
                    CsvTable.Rows.Add($"{item.NOCODE}", $"{item.NOCDE1}", $"{item.NOYSRV}", $"{item.KBSTKN}", $"{item.WorkFlag}");

                    //CSVテーブルの内容を出力
                    foreach (DataRow row in CsvTable.Rows)
                    {
                        Console.WriteLine($"2-4-2======{row["NOCODE"]},{row["NOCDE1"]},{row["NOYSRV"]},{row["KBSTKN"]},{row["WorkFlag"]}");
                    }
            ...
            try
            {
                string BaseDir = AppDomain.CurrentDomain.BaseDirectory;   //パス取得
                string workDirPath = Path.Combine(BaseDir, WorkPath);    //取得したパスにWorkPathのフォルダを作る為のパス作成

                if (!Directory.Exists(workDirPath))
                {
                    Directory.CreateDirectory(workDirPath);             //WorkPathフォルダが存在しない場合、作成
                }
                ...
                {
                    //  CSVのヘッダー行を書き込み
                    writer.WriteLine("コード番号, 枝番, 実行予算リビジョン,完了状況,完成フラグ");
                    foreach (DataRow row in CsvTable.Rows)
                    {
                        // カンマで結合して1行書き込み
                ...
                //ファイル作成場所の出力
                Console.WriteLine("ファイル作成場所: " + Path.GetFullPath(FromPath));
                ...
                string outDirPath = Path.Combine(BaseDir, OutPath);     //取得したパスにOutPathのフォルダ作る為のパスを作成

                if (!Directory.Exists(outDirPath))
                {
                    Directory.CreateDirectory(outDirPath);              //OutPathフォルダが存在しない場合、OutPathフォルダ作成
                }
            ...
            try
            {
                using (var conn = new OdbcConnection(connStr))
                {
                    conn.Open();
                    Console.WriteLine("接続成功");
                    string insertSql3 = "INSERT INTO FHBRR (SPIPGM, NOBRRN, KBBRRN, CTBRRN, STRENK, TXCOMT, VLPTBF, VLPTAF) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                    var cmd = new OdbcCommand(insertSql3, conn);

                    cmd.Parameters.Add("SPIPGM", OdbcType.VarChar).Value = "BAT240";
                    cmd.Parameters.Add("NOBRRN", OdbcType.VarChar).Value = CMSAIB;
                    cmd.Parameters.Add("KBBRRN", OdbcType.Int).Value = "5";
                    cmd.Parameters.Add("CTBRRN", OdbcType.VarChar).Value = RnkDate;
